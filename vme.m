...
%------------------ update omega_d     
    omega_d(n+1) = (omega_axis(T/2+1:T)*(abs(u_hat_d(n+1, T/2+1:T)).^2)')/sum(abs(u_hat_d(n+1,T/2+1:T)).^2);
    
    
%------------------ update lambda (dual ascent)     
    lambda(n+1,:) = lambda(n,:) + (tau*...
        (f_hat_onesided-(u_hat_d(n,:)))...
        ./(1+(Alpha.^2).*(omega_axis - omega_d(n)).^4));
    ...
    udiff = eps;
%------------------ 1st loop criterion    
    udiff = udiff + 1/T*(u_hat_d(n,:)-u_hat_d(n-1,:))*conj((u_hat_d(n,:)-u_hat_d(n-1,:)))';
